...
from tfTeams._factories.create_champions import ALL_CHAMPIONS
...
def generate_random_single_composition(
    all_compositions: List[None] | List[Comp], length_compositon: int
) -> Comp:
    while True:
        random_composition = Comp(
            champions=generate_random_list_champions(
                length_compositon=length_compositon
            )
        )
...
def generate_random_list_compositions(length_compositon: int) -> List[Comp]:
    ...
    for _ in range(1000):
        compositions.append(
            generate_random_single_composition(
                all_compositions=compositions, length_compositon=length_compositon
            )
        )
    return compositions


def new_composition_from_parents(
    first_parent_composition: Comp, second_parent_composition: Comp
) -> Comp:
    ...
    all_champions = list(
        set(champions_from_first_parent + champions_from_second_parent)
    )
...
def genetic_multiplication_composition(parent_composition: List[Comp]) -> List[Comp]:
    for couple_index in range(250):
        new_composition = new_composition_from_parents(
            first_parent_composition=parent_composition[couple_index],
            second_parent_composition=parent_composition[250 - couple_index],
        )
        if new_composition not in parent_composition:
            parent_composition.append(new_composition)
        else:
            parent_composition.append(
                generate_random_single_composition(
                    all_compositions=parent_composition,
                    length_compositon=len(parent_composition[0].champions),
                )
            )
...
def some_composition_mutate(all_compositions: List[Comp]) -> List[Comp]:
    random_composition_index, random_champion_index = randint(0, 59), randint(
        0, len(all_compositions[0].champions) - 1
    )
    all_compositions[random_composition_index].champions[random_champion_index] = (
        generate_random_champion(composition=all_compositions[random_composition_index])
    )
...
def best_genetic_composition(initial_compositions: List[Comp]) -> List[Comp]:
    ...
    while True:
        ...
        actual_compositions = genetic_multiplication_composition(
            parent_composition=actual_compositions
        )
        if randint(0, 5):
            actual_compositions = some_composition_mutate(
                all_compositions=actual_compositions
            )
...
def display_best_comps(best_comps: List[Comp]) -> None:
    for index, compositon in enumerate(best_comps):
        ...
        print(
            f"With headliner : {compositon.headliner}, we have {compositon.all_medals_from_comp(headliner=compositon.headliner)} medals"
        )
