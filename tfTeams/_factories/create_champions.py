...
from tfTeams.path_config import CONFIG_PATHS
...
def create_all_champions(champ_dataset_path: Path) -> List[StandardChampion]:
    ...
    for _, champion_row in champions_dataframe.iterrows():
        champion_synergies = [
            create_synergy_by_name(synergy_name=synergy_name)
            for synergy_name in champion_row["synergies"]
        ]
        all_champions.append(
            StandardChampion(
                name=champion_row["name"],
                cost=ChampionCost(champion_row["cost"]),
                synergies=champion_synergies,
            )
        )
    return all_champions


ALL_CHAMPIONS = create_all_champions(CONFIG_PATHS.config_paths.champions_path)
